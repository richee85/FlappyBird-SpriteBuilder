...
@implementation MainScene {
    ...
    CGPoint _cloudParalaxRatio;
    CGPoint _bushParalaxRatio;
    
    CCNode *_paralaxContainer;
    CCParallaxNode *_paralaxBackground;
...
- (void)didLoadFromCCB {
    ...
    _paralaxBackground = [CCParallaxNode node];
    [_paralaxContainer addChild:_paralaxBackground];
    
    _bushParalaxRatio = ccp(0.9, 1);
    _cloudParalaxRatio = ccp(0.5, 1);
    
    for (CCNode *bush in _bushes) {
        CGPoint offset = bush.position;
        [self removeChild:bush];
        [_paralaxBackground addChild:bush z:0 parallaxRatio:_bushParalaxRatio positionOffset:offset];
    }
    
    for (CCNode *cloud in _clouds) {
        CGPoint offset = cloud.position;
        [self removeChild:cloud];
        [_paralaxBackground addChild:cloud z:0 parallaxRatio:_cloudParalaxRatio positionOffset:offset];
    }
...
- (void)update:(CCTime)delta
{
    ...
    for (CCNode *ground in _grounds) {
        ...
        _paralaxBackground.position = ccp(_paralaxBackground.position.x - (character.physicsBody.velocity.x * delta), _paralaxBackground.position.y);
        
        // loop the bushes
        for (CCNode *bush in _bushes) {
            // get the world position of the bush
            CGPoint bushWorldPosition = [_paralaxBackground convertToWorldSpace:bush.position];
            ...
            if (bushScreenPosition.x <= (-1 * bush.contentSize.width)) {
                for (CGPointObject *child in _paralaxBackground.parallaxArray) {
                    if (child.child == bush) {
        ...
        for (CCNode *cloud in _clouds) {
            ...
            CGPoint cloudWorldPosition = [_paralaxBackground convertToWorldSpace:cloud.position];
